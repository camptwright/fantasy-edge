...
type TeamLine = { event_id: string; matchup: string; market: string; selection: string; line: number | null; price_american: number | null; source: string; model_probability: number | null; model_projection: number | null; fair_price_american: number | null; model_version: string | null; confidence: string | null; status: "qualified" | "uncalibrated" | "coverage_incomplete" };
  ...
  return <div className="space-y-6">
    ...
    {data && <>
      <section className="space-y-3"><div><p className="eyebrow">ESPN game markets</p><h2 className="mt-1 text-lg font-medium text-gray-100">Team lines and model state</h2></div>
        <Card className="overflow-x-auto p-0"><table className="w-full min-w-[850px] text-left text-sm"><thead className="text-xs uppercase tracking-wide text-gray-500"><tr><th className="px-4 py-3">Matchup</th><th>Market</th><th>Selection</th><th>Line</th><th>Price</th><th>Projection</th><th>Model / fair</th><th className="px-4">State</th></tr></thead><tbody>{data.team_lines.map((line, i) => <tr key={`${line.event_id}-${line.market}-${line.selection}-${i}`} className="border-t border-border/70"><td className="px-4 py-3 text-gray-300">{line.matchup}<span className="ml-2 text-xs uppercase text-gray-600">{line.source}</span></td><td className="py-3 text-gray-400">{line.market}</td><td className="py-3 text-gray-200">{line.selection}</td><td className="py-3 font-mono">{line.line ?? "—"}</td><td className="py-3 font-mono text-gray-400">{formatPrice(line.price_american)}</td><td className="py-3 font-mono text-gray-300">{line.model_projection ?? "—"}</td><td className="py-3 text-gray-300">{percent(line.model_probability)} <span className="ml-1 text-xs text-gray-500">{formatPrice(line.fair_price_american)}</span></td><td className="px-4 py-3"><StatusBadge status={line.status} /></td></tr>)}</tbody></table>{!data.team_lines.length && <EmptyState message="ESPN has not published NFL game lines in the current schedule window." />}</Card>
      </section>
