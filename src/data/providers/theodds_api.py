...
async def _guarded_get(path: str, params: dict[str, Any]) -> list[dict[str, Any]]:
    if await is_quota_exhausted():
    ...
    if remaining_header is not None:
        ...
        if remaining < settings.odds_api_quota_floor:
            ...
            await set_quota_exhausted(remaining)
    else:
...
async def get_odds(sport: str, *, regions: str = "us") -> list[dict[str, Any]]:
...
async def get_scores(sport: str, *, days_from: int = 1) -> list[dict[str, Any]]:
